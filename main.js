...
function CreateSurfaceData()
{
    ...
    for (let i = 0; i < numPointsU; i++) {
        ...
        for (let j = 0; j < numPointsV; j++) {
            const u1 = (i * 2 * Math.PI) / numPointsU;
            const u2 = ((i + 1) * 2 * Math.PI) / numPointsU;
            const v1 = (j / numPointsV) * rMax;
            const v2 = ((j + 1) / numPointsV) * rMax;
...
function CreateTextureCoordinates(numPointsU, numPointsV) {
    ...
    for (let i = 0; i < numPointsU; i++) {
        for (let j = 0; j < numPointsV; j++) {
            const u1 = i / (numPointsU - 1);
            const v1 = j / (numPointsV - 1);
